...
- name: Create Workers Instance. It will Automatically be Set by the Monitor as the Primary Node with Read-Write Capability (PGDATA = /var/lib/pgpsql/ha)
  become_user: postgres
  shell: |
    source {{ pgsql.base_dir }}/.bash_profile && {{ common.pgsql_bin_dir }}/pg_autoctl create postgres \
    --pgdata '{{ pgsql.base_dir }}/{{ _pgfailover.workers_database }}' --auth trust --ssl-self-signed \
...
- name: Create Systemd Service File of Worker Instances
  become_user: postgres
  shell: |
    source {{ pgsql.base_dir }}/.bash_profile && {{ common.pgsql_bin_dir }}/pg_autoctl -q show systemd \
    --pgdata '{{ pgsql.base_dir }}/{{ _pgfailover.workers_database }}' > pgautofailover.service
